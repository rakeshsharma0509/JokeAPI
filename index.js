...
app.put("/jokes/:id",(req, res)=>{
  ...
  const exist=jokes.find((jokes)=>
  {
    if(nid===jokes.id) return true;
    return false
  });

  if(!exist || !req.body.text || !req.body.type )
  {
...
app.patch("/jokes/:id", (req, res)=>{
  const nid=parseInt(req.params.id);
  const existingJoke=jokes.find((joke)=>joke.id===nid);
  const njoke={
    id:nid,
  ...
  const searchIndex=jokes.findIndex((joke)=> joke.id===nid);
  jokes[searchIndex]=njoke;
  res.json(njoke);
...
app.delete("/jokes/:id", (req, res)=>{
  const nid=parseInt(req.params.id);
  const idx=jokes.findIndex((jokes)=>jokes.id===nid);
  if(idx>-1)
  {
    jokes.splice(idx,1);
    res.sendStatus(200);
    ...
    res.status(404).json({error: "invalid joke id"});
  }
});

//8. DELETE All jokes
app.delete("all/", (req, res)=>{
  const ukey=req.query.key;
  if(ukey===masterKey)
  {
    jokes=[];
    res.sendStatus(200);
    ...
    res.status(404).send("invalid request you are not auhtoirsed");
  }
})
